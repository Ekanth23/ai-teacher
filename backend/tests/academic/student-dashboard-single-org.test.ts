...
async function createUser(label: string) {
  const result = await pool.query(
    ...
    [`${unique(`dash_${label}`)}@example.com`, `Dashboard ${label}`]
  );
...
async function createOrganization(ownerUserId: string, label: string) {
  const result = await pool.query(
    ...
    [`Dashboard ${label}`, unique(`dash_org_${label}`), ownerUserId]
  );
...
async function createClass(organizationId: string, ownerUserId: string, label: string) {
  const result = await pool.query(
    ...
    [organizationId, `Dashboard class ${label}`, ownerUserId]
  );
...
async function createSubject(organizationId: string, label: string) {
  const result = await pool.query(
    ...
    [organizationId, `Dashboard subject ${label}`, unique(`dashsubj_${label}`)]
  );
...
async function createStudent(organizationId: string, userId: string, classId: string, fullName: string, gradeLevel: string) {
...
describe("real student dashboard path (single active organization, no header)", () => {
  it("serves the dashboard without X-Organization-Id", async () => {
    const studentUserId = await createUser("student");
    const organizationId = await createOrganization(studentUserId, "school");
    await addMember(studentUserId, organizationId, "STUDENT");
    const classId = await createClass(organizationId, studentUserId, "8a");
    const subjectId = await createSubject(organizationId, "math");
    await mapSubjectToClass(organizationId, classId, subjectId);
    const studentId = await createStudent(organizationId, studentUserId, classId, "Alice Student", "8");
    ...
    expect(res.body.current_class.id).toBe(classId);
    expect(res.body.subjects).toHaveLength(1);
    expect(res.body.subjects[0].id).toBe(subjectId);
  });

  it("serves the student's classes without X-Organization-Id", async () => {
    const studentUserId = await createUser("classes");
    const organizationId = await createOrganization(studentUserId, "school");
    await addMember(studentUserId, organizationId, "STUDENT");
    const classId = await createClass(organizationId, studentUserId, "8a");
    await createStudent(organizationId, studentUserId, classId, "Alice Student", "8");
