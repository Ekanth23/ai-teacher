...
async function createUser(label: string) {
  const result = await pool.query(
    ...
    [`${unique(`p071_${label}`)}@example.com`, `Phase 071 ${label}`]
  );
...
async function createOrganization(ownerUserId: string, label: string) {
  const result = await pool.query(
    ...
    [`Phase 071 ${label}`, unique(`p071_org_${label}`), ownerUserId]
  );
...
async function createClass(organizationId: string, ownerUserId: string, label: string) {
  const result = await pool.query(
    ...
    [organizationId, `Phase 071 class ${label}`, ownerUserId]
  );
...
async function createSubject(organizationId: string, label: string) {
  const result = await pool.query(
    ...
    [organizationId, `Phase 071 subject ${label}`, unique(`p071subj_${label}`)]
  );
...
async function createStudent(organizationId: string, userId: string, classId: string, fullName = "Phase 071 Student") {
...
function review(token: string, organizationId: string, assignmentId: string, submissionId: string, decision: string) {
  return request(app)
...
function setFeedback(token: string, organizationId: string, assignmentId: string, submissionId: string, body: Record<string, unknown>) {
  return request(app)
    .patch(`/api/assignments/${assignmentId}/submissions/${submissionId}/feedback`)
    .set(auth(token, organizationId))
    .send(body);
}
...
describe("US-071 teacher adds feedback to a submission", () => {
  it("allows a class teacher to add feedback", async () => {
    const f = await fixture();
    const { assignmentId, submissionId } = await openAssignmentWithSubmission(f);

    const res = await setFeedback(f.teacherToken, f.organizationId, assignmentId, submissionId, { feedback: "Good work." });
    expect(res.status).toBe(200);
    expect(res.body.submission.feedback).toBe("Good work.");
  });

  it("allows a subject teacher to add feedback", async () => {
    const f = await fixture();
    ...
    const res = await setFeedback(createAccessToken(subjectTeacherUserId), f.organizationId, assignmentId, submissionId, { feedback: "Good work." });
    expect(res.status).toBe(200);
    expect(res.body.submission.feedback).toBe("Good work.");
  });

  it("persists the feedback", async () => {
    const f = await fixture();
    const { assignmentId, submissionId } = await openAssignmentWithSubmission(f);
    ...
    const db = await pool.query("SELECT feedback FROM submissions WHERE id = $1", [submissionId]);
    expect(db.rows[0].feedback).toBe("Good work.");
  });

  it("returns the feedback in the response", async () => {
    const f = await fixture();
    const { assignmentId, submissionId } = await openAssignmentWithSubmission(f);
  ...
  it("allows existing feedback to be updated", async () => {
    const f = await fixture();
    const { assignmentId, submissionId } = await openAssignmentWithSubmission(f);

    const first = await setFeedback(f.teacherToken, f.organizationId, assignmentId, submissionId, { feedback: "Good work." });
    expect(first.status).toBe(200);

    const second = await setFeedback(f.teacherToken, f.organizationId, assignmentId, submissionId, { feedback: "Please improve question 4." });
    expect(second.status).toBe(200);
    expect(second.body.submission.feedback).toBe("Please improve question 4.");

    const db = await pool.query("SELECT feedback FROM submissions WHERE id = $1", [submissionId]);
    expect(db.rows[0].feedback).toBe("Please improve question 4.");
  });

  it("allows existing feedback to be cleared with null", async () => {
    const f = await fixture();
    const { assignmentId, submissionId } = await openAssignmentWithSubmission(f);
    ...
    const cleared = await setFeedback(f.teacherToken, f.organizationId, assignmentId, submissionId, { feedback: null });
    expect(cleared.status).toBe(200);
    expect(cleared.body.submission.feedback).toBeNull();

    const db = await pool.query("SELECT feedback FROM submissions WHERE id = $1", [submissionId]);
    expect(db.rows[0].feedback).toBeNull();
  });

  it("allows a submission to have no feedback", async () => {
    const f = await fixture();
    const { assignmentId, submissionId } = await openAssignmentWithSubmission(f);
  ...
  it("allows feedback to exist when the decision is null", async () => {
    const f = await fixture();
    const { assignmentId, submissionId } = await openAssignmentWithSubmission(f);

    const res = await setFeedback(f.teacherToken, f.organizationId, assignmentId, submissionId, { feedback: "Please review your answer." });
    expect(res.status).toBe(200);
    expect(res.body.submission.feedback).toBe("Please review your answer.");
    expect(res.body.submission.decision).toBeNull();
  });

  it("allows feedback to coexist with an ACCEPTED decision", async () => {
    const f = await fixture();
    const { assignmentId, submissionId } = await openAssignmentWithSubmission(f);

    await review(f.teacherToken, f.organizationId, assignmentId, submissionId, "ACCEPTED");
    const res = await setFeedback(f.teacherToken, f.organizationId, assignmentId, submissionId, { feedback: "Great work." });
    expect(res.status).toBe(200);
    expect(res.body.submission.decision).toBe("ACCEPTED");
    expect(res.body.submission.feedback).toBe("Great work.");
  });

  it("allows feedback to coexist with a REDO_REQUIRED decision", async () => {
    const f = await fixture();
    const { assignmentId, submissionId } = await openAssignmentWithSubmission(f);

    await review(f.teacherToken, f.organizationId, assignmentId, submissionId, "REDO_REQUIRED");
    const res = await setFeedback(f.teacherToken, f.organizationId, assignmentId, submissionId, { feedback: "Please correct questions 3 and 4." });
    expect(res.status).toBe(200);
    expect(res.body.submission.decision).toBe("REDO_REQUIRED");
    expect(res.body.submission.feedback).toBe("Please correct questions 3 and 4.");
  });

  it("adding feedback does not change the decision", async () => {
    const f = await fixture();
    const { assignmentId, submissionId } = await openAssignmentWithSubmission(f);
  ...
  it("clearing feedback does not change the decision", async () => {
    const f = await fixture();
    const { assignmentId, submissionId } = await openAssignmentWithSubmission(f);

    await review(f.teacherToken, f.organizationId, assignmentId, submissionId, "REDO_REQUIRED");
    await setFeedback(f.teacherToken, f.organizationId, assignmentId, submissionId, { feedback: "Redo this." });
    const res = await setFeedback(f.teacherToken, f.organizationId, assignmentId, submissionId, { feedback: null });
    expect(res.status).toBe(200);
    expect(res.body.submission.decision).toBe("REDO_REQUIRED");
    expect(res.body.submission.feedback).toBeNull();
  });

  it("keeps the submission content unchanged", async () => {
    const f = await fixture();
    const { assignmentId, submissionId } = await openAssignmentWithSubmission(f);
  ...
  it("keeps the student identity unchanged", async () => {
    const f = await fixture();
    const { assignmentId, submissionId } = await openAssignmentWithSubmission(f);
  ...
  it("rejects an invalid assignment id", async () => {
    const f = await fixture();
    const res = await setFeedback(f.teacherToken, f.organizationId, "not-a-uuid", "00000000-0000-4000-8000-000000000000", { feedback: "Good work." });
  ...
  it("rejects an invalid submission id", async () => {
    ...
    const res = await setFeedback(f.teacherToken, f.organizationId, assignmentId, "not-a-uuid", { feedback: "Good work." });
    expect(res.status).toBe(400);
  });
  ...
  it("rejects a non-string feedback value", async () => {
    ...
    const res = await setFeedback(f.teacherToken, f.organizationId, assignmentId, submissionId, { feedback: 123 });
    expect(res.status).toBe(400);
  });

  it("rejects excessively long feedback", async () => {
    const f = await fixture();
    const { assignmentId, submissionId } = await openAssignmentWithSubmission(f);

    const res = await setFeedback(f.teacherToken, f.organizationId, assignmentId, submissionId, { feedback: "a".repeat(2001) });
    expect(res.status).toBe(400);
  });
  ...
  it("rejects an empty request body", async () => {
    ...
    const res = await setFeedback(f.teacherToken, f.organizationId, assignmentId, submissionId, {});
    expect(res.status).toBe(400);
  });
  ...
  it("returns 404 for a nonexistent submission", async () => {
    ...
    const res = await setFeedback(f.teacherToken, f.organizationId, assignmentId, "00000000-0000-4000-8000-000000000000", { feedback: "Good work." });
  ...
  it("returns 404 for a submission belonging to another assignment", async () => {
    ...
    const res = await setFeedback(f.teacherToken, f.organizationId, a1.assignmentId, a2.submissionId, { feedback: "Good work." });
    expect(res.status).toBe(404);
  });
  ...
  it("returns 404 for a cross-tenant submission", async () => {
    ...
    const res = await setFeedback(f.teacherToken, f.organizationId, assignmentId, otherSub.body.submission.id, { feedback: "Good work." });
    expect(res.status).toBe(404);
  });
  ...
  it("rejects a teacher not authorized for the assignment", async () => {
    ...
    const res = await setFeedback(createAccessToken(outsiderUserId), f.organizationId, assignmentId, submissionId, { feedback: "Good work." });
  ...
  it("rejects a teacher authorized for a different subject", async () => {
    ...
    const res = await setFeedback(createAccessToken(otherTeacherUserId), f.organizationId, assignmentId, submissionId, { feedback: "Good work." });
    expect(res.status).toBe(403);
  });
  ...
  it("rejects a student role", async () => {
    ...
    const res = await setFeedback(f.studentToken, f.organizationId, assignmentId, submissionId, { feedback: "Good work." });
    expect(res.status).toBe(403);
  });
  ...
  it("rejects a TEACHER-role user without an active teacher profile", async () => {
    ...
    const res = await setFeedback(createAccessToken(noProfileUserId), f.organizationId, assignmentId, submissionId, { feedback: "Good work." });
    expect(res.status).toBe(403);
  });

  it("rejects unauthenticated requests", async () => {
    const res = await request(app)
      .patch("/api/assignments/00000000-0000-4000-8000-000000000000/submissions/00000000-0000-4000-8000-000000000000/feedback")
  ...
  it("ignores client-supplied identity and organization fields", async () => {
    ...
    const res = await request(app)
      ...
      .send({ feedback: "Good work.", student_id: studentBId, organization_id: "00000000-0000-4000-8000-000000000000" });
  ...
  it("does not create or modify marks", async () => {
    const f = await fixture();
    const { assignmentId, submissionId } = await openAssignmentWithSubmission(f);

    const res = await setFeedback(f.teacherToken, f.organizationId, assignmentId, submissionId, { feedback: "Good work.", marks: 85 });
    expect(res.status).toBe(200);
    expect(res.body.submission.feedback).toBe("Good work.");
  ...
  it("does not create a redo or resubmission", async () => {
    const f = await fixture();
    const { assignmentId, submissionId } = await openAssignmentWithSubmission(f);

    const res = await setFeedback(f.teacherToken, f.organizationId, assignmentId, submissionId, { feedback: "Please redo this." });
    expect(res.status).toBe(200);
